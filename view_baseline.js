...
perm_feedback_div = $(
  '<div id="permdialog_feedback_text" class="section" style="color:#444; background:#f5f5f5; border:1px solid #ccc; padding:8px; margin-top:8px;">Select a user to view permission details.</div>',
);
...
file_permission_users = define_single_select_list(
  ...
  function (selected_user, e, ui) {
    ...
    let selected_elem = file_permission_users.find(".ui-selected");
    let has_inherited_permissions = selected_elem.attr("inherited") === "true";

    if (has_inherited_permissions) {
      $("#permdialog_feedback_text").text(
        "This user currently has inherited permissions. Changes here may not override parent settings.",
      );
    } else {
      $("#permdialog_feedback_text").text(
        "This user has direct permissions on this object.",
      );
    }
  },
);
    ...
    buttons: {
      Yes: {
        ...
        click: function () {
          ...
          let filepath = perm_dialog.attr("filepath"); // Remove all the permissions:
...
perm_remove_user_button.click(function () {
  ...
  let selected_username = file_permission_users.attr("selected_item"); // Get the actual element that we want to remove from the user list:
  ...
  if (has_inherited_permissions) {
    $(".cant_remove_username").text(selected_username);
    $("#permdialog_feedback_text").text(
      "This user currently has inherited permissions and cannot be removed here. Turn off inheritance in Advanced settings first.",
    );
    cant_remove_dialog.dialog("open");
  } else {
    $("#permdialog_feedback_text").text(
      "You are removing a direct permission entry for this user.",
    );
...
perm_dialog.append(perm_feedback_div);
...
function make_all_users_list(id_prefix, attr_set_id, height = 80) {
  ...
  for (let username in all_users) {
    ...
    all_user_list.append(`<div class="ui-widget-content" id="${id_prefix}_${username}" username="${username}">
...
function open_advanced_dialog(file_path) {
  let file_obj = path_to_file[file_path]; // set file path in UI:

  $("#adv_perm_filepath").text(file_path);
  $("#adv_owner_filepath").text(file_path);
  $("#adv_effective_filepath").text(file_path);
  $("#advdialog").attr("filepath", file_path); // clear dynamic content:
  ...
  if ($("#adv_inheritance_feedback").length === 0) {
    $("#adv_perm_inheritance_div").append(
      '<div id="adv_inheritance_feedback" style="margin-top:6px; color:#444;">Changes here may not override parent settings while inheritance is enabled.</div>',
    );
  }
  if (file_obj.using_permission_inheritance) {
    $("#adv_inheritance_feedback").text(
      "Inheritance is enabled. Changes here may not override parent settings.",
    );
  } else {
    $("#adv_inheritance_feedback").text(
      "Inheritance is disabled. Permissions on this object are now controlled directly here.",
    );
  ...
  for (let u in users) {
    ...
    for (let ace_type in grouped_perms) {
      for (let perm in grouped_perms[ace_type]) {
        $("#adv_perm_table")
          .append(`<tr id="adv_perm_${file_obj.filename}__${u}_${ace_type}_${perm}">
...
                </tr>`);
  ...
  $("#adv_owner_user_list").append(all_user_list); // open dialog:
...
function update_effective_user() {
  ...
  if (
    ...
    let file = path_to_file[filepath]; // for each possible permission value
  ...
  for (let mutation of mutationsList) {
    if (mutation.type === "attributes") {
      if (mutation.attributeName === "selected_user") {
        ...
        let file_obj = path_to_file[filepath]; // get rid of previous checkboxes:
        $(".perm_entry_checkcell").empty(); // by default, put unchecked checkboxes everywhere:
